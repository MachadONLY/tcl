...
const SHAPE = Object.freeze({ GK: 1, DEF: 4, MID: 4, FWD: 2 });
...
function lineupFor(career, code) {
  const employed = squadForWorld(career, code, PLAYER_BY_ID);
  if (code !== career.clubCode) return pickLineup(employed);
  ...
  const remainder = pickLineup(employed.filter(player => !selected.some(chosen => chosen.id === player.id)));
  return [...selected, ...remainder].slice(0, 11);
}

function aiTactics(code) {
  const elo = Number(TEAM_ELO[code]) || 1750;
  const base = defaultTactics();
  return normalizeTactics({
    ...base,
    mentality: elo >= 1930 ? 'Positiva' : elo < 1700 ? 'Cautelosa' : 'Equilibrada',
    pressing: clamp(55 + Math.round((elo - 1750) / 18), 40, 78),
    tempo: clamp(58 + Math.round((elo - 1750) / 22), 44, 76),
    defensiveLine: clamp(53 + Math.round((elo - 1750) / 20), 40, 73),
    buildUp: elo >= 1900 ? 'Curta' : elo < 1690 ? 'Direta' : 'Equilibrada',
    afterLoss: elo >= 1900 ? 'Contrapressão' : elo < 1690 ? 'Recompor' : 'Contextual'
  });
}

function profile(career, code, home) {
  const players = lineupFor(career, code);
  ...
  return { code, user, players, tactics, metrics, condition, rating, power, attack, defence };
}
...
function weightedPlayer(random, players, assisting = false) {
  const rows = players.map(player => {
    const weight = player.group === 'FWD' ? 5.4 : player.group === 'MID' ? (assisting ? 3.3 : 2.25) : player.group === 'DEF' ? 0.55 : 0.06;
    return { player, weight: weight * clamp((Number(player.rating) || 70) / 75, 0.72, 1.28) };
  });
...
function goalEvents(random, goals, side, players) {
  ...
  for (let index = 0; index < goals; index += 1) {
    ...
    const assist = random() > 0.2 ? weightedPlayer(random, possibleAssists, true) : null;
    events.push({
      type: 'goal',
      minute,
      side,
...
export function simulateWorldFixture(career, fixture) {
  ...
  const home = profile(career, fixture.home, true);
  const away = profile(career, fixture.away, false);
  const homeEdge = (home.attack - away.defence) / 17;
  const awayEdge = (away.attack - home.defence) / 17;
  const homeXg = clamp(1.18 + homeEdge * 0.23 + (random() - 0.5) * 0.28, 0.22, 4.0);
  const awayXg = clamp(1.00 + awayEdge * 0.23 + (random() - 0.5) * 0.28, 0.18, 3.8);
  ...
  const controlDelta = (home.metrics.control - away.metrics.control) * 0.23 + (home.power - away.power) * 0.45;
  const possession = clamp(Math.round(50 + controlDelta + (random() - 0.5) * 6), 28, 72);
  ...
  return {
    ...
    tactical: {
      home: { metrics: home.metrics, load: home.metrics.intensity || 62, plan: home.user ? career.tactics.activePlan || 'A' : 'AI' },
      away: { metrics: away.metrics, load: away.metrics.intensity || 62, plan: away.user ? career.tactics.activePlan || 'A' : 'AI' }
    }
  };
}
